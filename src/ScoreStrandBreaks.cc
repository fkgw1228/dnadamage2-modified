...
G4bool ScoreStrandBreaks::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
    ...
    if (isSugar || isPhosphate) {
        ...
        // volumeName format: "phy_ellipsoid_<chainId>_<nucleotideId>_<compoundName>"
        // After SplitByUnderscore: [0]="phy", [1]="ellipsoid", [2]=chainId, [3]=nucleotideId, [4]=compoundName
        ...
        G4int dnaId = aTouchable->GetReplicaNumber(1);
        G4String chainId = tokens[2];
        G4int nucleotideId = std::stoi(tokens[3]);
        G4String compoundName = tokens[4];
        
        G4ThreeVector pos = aStepPoint->GetPosition();

        DNALocation location{eventId, dnaId, chainId, nucleotideId, compoundName};
        if (fDirectDamageMap.find(location) == fDirectDamageMap.end())
            fDirectDamageMap[location]
                .push_back(GetApproximateAtomName(dnaId, chainId, nucleotideId, compoundName, pos));
    ...
    if (!fDNAInserted) {
        ...
        for (auto& [dnaId, dnaStructure] : fpDetector->GetDNAMap()) {
                    ...
                    for (auto& [compoundName, compound] : nucleotide.GetCompounds()) {
                        ...
                        if(compoundName=="deoxyribose") {
                            for(Atom atom : atoms) {
                                ...
                                if(atom_name=="H1\'")         InsertMolecule("H1", location, position);
                                else if(atom_name=="H2\'")    InsertMolecule("H2a", location, position);
                                else if(atom_name=="H2\'\'")  InsertMolecule("H2b", location, position);
                                else if (atom_name=="H3\'")   InsertMolecule("H3", location, position);
                                else if (atom_name=="H4\'")   InsertMolecule("H4", location, position);
                                else if (atom_name=="H5\'")   InsertMolecule("H5a", location, position);
                                else if (atom_name=="H5\'\'") InsertMolecule("H5b", location, position);
                            }
                        }
                    }
                }
...
std::vector<G4String> ScoreStrandBreaks::SplitByUnderscore(const G4String &s) {
...
G4String ScoreStrandBreaks::GetApproximateAtomName(G4int dnaId, G4String chainId, G4int nucleotideId,
                                                   G4String compound_name, G4ThreeVector position) {
    DNAStructure dnaStructure = fpDetector->GetDnaStructure(dnaId);
    DNA dna = dnaStructure.GetDNA();
    std::vector<Atom> atoms = dna.GetStrand(chainId)
                                  .GetNucleotide(nucleotideId)
                                  .GetCompound(compound_name)
                                  .GetAtoms();
    ...
    for (auto atom : atoms) {
        ...
        G4double distance = (atom.GetPosition() - position).mag() - radius;
...
void ScoreStrandBreaks::InsertMolecule(G4String moleculeName, DNALocation location, G4ThreeVector position) {
...
void ScoreStrandBreaks::EndOfEvent(G4HCofThisEvent*)
{
    ...
    // Absorbed Dose
    G4double volume  = 4.0/3 * 3.14159 * (*fRadius) * (*fRadius) * (*fRadius);
    ...
    for(size_t i = 0; i < InsertedTracks.size(); i++) {
        if(InsertedTracks[i]->GetTrackStatus() == fStopAndKill) {
            std::pair<DNALocation, G4String> insertedPositions = fInsertedMolecules[i];
            DNALocation location = insertedPositions.first;
...
void ScoreStrandBreaks::AbsorbResultsFromWorkerScorer(G4VPrimitiveScorer* workerScorer)
{
    ...
    for (auto& damageLocation : WorkerDirectDamageMap) {
        DNALocation location = damageLocation.first;
...
void ScoreStrandBreaks::WriteWithAnalysisManager(G4VAnalysisManager* analysisManager)
{
    ...
    analysisManager->CreateNtupleIColumn(fNtupleID, "DNAId");
    analysisManager->CreateNtupleSColumn(fNtupleID, "StrandId");
    analysisManager->CreateNtupleIColumn(fNtupleID, "NucleotideId");
    ...
    for (auto& damageLocation : fDirectDamageMap) {
        ...
        for (auto atomName : damageLocation.second) {
            ...
            analysisManager->FillNtupleIColumn(fNtupleID, 1, location.dnaID);
            analysisManager->FillNtupleSColumn(fNtupleID, 2, location.chainID);
            analysisManager->FillNtupleIColumn(fNtupleID, 3, location.nucleotideID);
...
void ScoreStrandBreaks::ASCII(G4double LET, G4double LET_STD)
{
    ...
    dna << "#" << std::setw(14) << "Event-ID"  
               << std::setw(14) << "DNA-ID" 
               << std::setw(14) << "Chain-ID"
               << std::setw(14) << "Nucleotide-ID" 
               << std::setw(20) << "Compound"
               << std::setw(20) << "Atom/Molecule"
               << std::setw(14) << "Break-ID"
               << std::setw(14) << "Dose (Gy) "<< G4endl;

    for (auto damageLocation : fDirectDamageMap) {
        DNALocation location = damageLocation.first;
        for (auto atomName : damageLocation.second) {
            dna << std::setw(14) << location.eventID
                << std::setw(14) << location.dnaID
                << std::setw(14) << location.chainID
                << std::setw(14) << location.nucleotideID
                << std::setw(20) << location.compoundName
                << std::setw(20) << atomName
                << std::setw(14) << 1
